...
function loadCache() {
  try {
    ...
    // Descartar cache si no tiene datos utiles
    if (!data?.personas?.length) return null
    return data
  } catch { return null }
}
...
export default function Dashboard({ isAdmin = true, isCoach = false }) {
  ...
  const statItems = [
    {label:'Atletas Adultos', val:adultos, pct:pct(adultos,personas.length), color:'#1d4ed8', bg:'#eff6ff', icon:'adulto', sub:'del total'},
    {label:'Atletas Ninos', val:ninos, pct:pct(ninos,personas.length), color:'#7c3aed', bg:'#fdf4ff', icon:'nino', sub:'del total'},
    {label:'Mujeres', val:mujeres, pct:pct(mujeres,personas.length), color:'#db2777', bg:'#fdf2f8', icon:'mujer', sub:'del total'},
    ...
    <div className="content">
      ...
      <div className="kpi-grid">
        {[
          ...
          ...((isAdmin || isCoach) && saldoMovimientos !== null ? [{label:'Saldo en caja', val:formatMoney(saldoMovimientos), sub:'ingresos menos egresos', icon:'ti-wallet', cls: saldoMovimientos >= 0 ? 'green' : 'red', small:true}] : []),
        ].map((k,i) => (
      ...
      <div className="two-col">
        ...
        <div className="card">
          ...
          <div style={{position:'relative',height:200}}>
            <Doughnut data={{labels:['Al dia','Moroso','Parcial'],datasets:[{data:[alDia,morosos,parcial],backgroundColor:['#16a34a','#dc2626','#d97706'],borderWidth:2}]}}
      ...
      {/* Estadisticas */}
      <div className="card">
        <div className="card-title"><i className="ti ti-chart-dots"></i>Estadisticas del club</div>
      ...
      <div className="two-col">
            ...
            : morososList.map(s => {
                ...
                return (
                  <div className="moroso-row" key={s.id_caif}>
                    ...
                    <span className={`badge ${s.atleta==='Atleta Nino'?'nino':'adulto'}`} style={{marginRight:6}}>{s.atleta==='Atleta Nino'?'Nino':'Adulto'}</span>
        ...
        {cumpleaneros.length > 0 ? (
          ...
          <div className="card" style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
            <div className="empty"><i className="ti ti-cake"></i>Sin cumpleanos este mes</div>
          </div>
        )}
      </div>
    </div>
